...
class AIRecognizePro(_PluginBase):
    plugin_name = "AI识别增强 自用定制版"
    plugin_desc = "基于 liuyuexi1987 的「AI识别增强」fork 定制自用：复用 MoviePilot 当前 LLM 配置，在原生识别失败后做本地结构化识别兜底，并交回原生链路继续二次识别；识别成功后可自动沉淀窄作用域识别词。感谢原作者 liuyuexi1987 的开源实现。"
    # 不再引用原作者仓库的图片资源，改用内置 mdi 图标，避免误以为使用了上游发布资产
    plugin_icon = "mdi-robot-outline"
    plugin_version = "1.0.2"
    plugin_author = "zhuzhug"
    plugin_level = 1
    author_url = "https://github.com/liuyuexi1987"
    plugin_config_prefix = "airecognizepro_"
    plugin_order = 41
    auth_level = 1
    ...
    # 识别成功后是否把"成功方式"自动沉淀为自定义识别词规则（默认关闭）
    _auto_write_identifier = False
    # 自动写入使用的最低建议置信度；LLM 给出低置信度建议时直接跳过
    _auto_write_min_confidence = 0.8
    # 保护 CustomIdentifiers 的读取-合并-写回不被并发请求交错覆盖
    ...
    def init_plugin(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._auto_write_identifier = bool(config.get("auto_write_identifier", False))
        self._auto_write_min_confidence = self._safe_float(
            config.get("auto_write_min_confidence"), 0.8
        )
        ...
        if self._clear_failed_samples_once:
            ...
            logger.info(f"[AI识别增强 自用定制版] 已按配置清空失败样本 {cleared} 条")
    ...
    def _build_config(self, overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        config = {
            ...
            "auto_write_identifier": self._auto_write_identifier,
            "auto_write_min_confidence": self._auto_write_min_confidence,
        }
        if overrides:
    ...
    def _register_events(self) -> None:
        try:
            ...
            logger.warning(f"[AI识别增强 自用定制版] 注册链式识别事件失败: {exc}")
    ...
    def _record_failed_sample(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(f"[AI识别增强 自用定制版] 写入失败样本失败: {exc}")
    ...
    def _record_llm_error(self, title: str, path: str, meta_hint: Dict[str, Any], error: Any, provenance: Optional[Dict[str, str]] = None) -> None:
        try:
            ...
            logger.warning(f"[AI识别增强 自用定制版] 写入 LLM 错误诊断记录失败: {exc}")
    ...
    def _read_llm_errors(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"[AI识别增强 自用定制版] 读取 LLM 错误诊断记录失败: {exc}")
            return []
        if limit > 0:
            rows = rows[-limit:]
        rows.reverse()
    ...
    def _read_failed_samples(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"[AI识别增强 自用定制版] 读取失败样本失败: {exc}")
            return []
        if limit > 0:
            rows = rows[-limit:]
        rows.reverse()
...
""",
    ...
    def _parse_json_model(model_cls: Any, content: Any) -> Any:
        ...
        text = AIRecognizePro._extract_json_text(content)
        if not text:
    ...
    def _remove_custom_identifiers(self, lines: List[str]) -> Dict[str, Any]:
        """从 CustomIdentifiers 中移除指定行，用于自动写入回放失败后的回滚。"""
        with self._identifier_lock:
            existing = self._get_custom_identifiers()
    ...
    def _auto_identifier_target_ok(self, target: Dict[str, Any]) -> Tuple[bool, str]:
        """目标必须足够明确才允许自动写入，模糊目标只记录不写入。"""
        target = target or {}
        ...
        if media_type == "unknown":
            return False, "类型不明"
        if not self._safe_int(target.get("tmdb_id"), 0):
            return False, "缺少 TMDB ID"
        if media_type == "tv" and not self._safe_int(target.get("season"), 0):
            return False, "剧集缺少季号"
        if media_type == "tv" and not self._safe_int(target.get("episode"), 0):
            return False, "剧集缺少集号"
        return True, ""

    def _auto_write_identifier_rule(self, title: str, path: str, result: Dict[str, Any]) -> Dict[str, Any]:
        """
        AI 识别成功后，把"成功方式"沉淀为自定义识别词规则。
        流程：生成建议 -> 目标明确性校验 -> 写入 -> 用原生识别词链路回放验证 -> 失败自动回滚。
        任何异常都只记录日志，绝不影响主识别链路。
        """
        try:
            suggested = self._suggest_identifiers({"auto_write": True, "title": title, "path": path})
            if not suggested.get("success"):
                return {"success": False, "stage": "suggest", "message": suggested.get("message", "建议生成失败")}

            data = suggested.get("data") or {}
            target = data.get("target") or {}
            ok, reason = self._auto_identifier_target_ok(target)
            if not ok:
                return {
                    "success": False,
                    "stage": "target",
                    "message": f"目标不明确，跳过自动写入：{reason}",
                    "target": target,
                }

            candidates = [item for item in (data.get("suggestions") or []) if item.get("lines")]
            min_conf = self._auto_write_min_confidence
            ...
            if not chosen:
                return {"success": False, "stage": "candidates", "message": "没有可直接写入的候选规则"}
            ...
            if not added:
                return {
                    "success": False,
                    "stage": "dedupe",
                    "message": "规则已存在或与其他规则重复，未写入",
                    "rule": chosen.get("rule"),
                    "target": target,
                }

            # 回放验证：此时规则已进入 CustomIdentifiers，走原生识别词预处理链路复查
            verify = self._preview_current_identifiers(title=title, target=target)
            passed = bool(verify.get("applied")) and bool(verify.get("matched_target"))
            if not passed:
                rolled_back = self._remove_custom_identifiers(added)
                logger.warning(
                    f"[AI识别增强 自用定制版] 自动识别词回放未命中目标，已回滚 "
                    f"{rolled_back.get('removed_count', 0)} 行: {chosen.get('rule')}"
                )
                return {
                    "success": False,
                    "stage": "verify",
                    "message": "回放未命中目标结果，已自动回滚",
                    "rule": chosen.get("rule"),
                    "target": target,
                    "verify": verify,
                }
            ...
            if self._debug:
                logger.info(f"[AI识别增强 自用定制版] 自动沉淀识别词成功: {title} -> {chosen.get('rule')}")
            return {
                "success": True,
                "stage": "written",
                "rule": chosen.get("rule"),
                ...
                "added": added,
                "total_count": apply_result.get("total_count"),
                "target": target,
                "verify": verify,
            }
        except Exception as exc:
            logger.warning(f"[AI识别增强 自用定制版] 自动沉淀识别词异常: {exc}")
            return {"success": False, "stage": "error", "message": str(exc)}
    ...
    def _verify_guess(self, title: str, path: str, guess: AIRecognitionGuess) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self._debug:
                logger.warning(f"[AI识别增强 自用定制版] 二次校验失败: {exc}")
            return None
    ...
    def _recognize(
        ...
        try:
            ...
            if record_failed_sample:
                if is_title_only and not self._save_title_only_samples:
                    if self._debug:
                        logger.info(f"[AI识别增强 自用定制版] 跳过保存仅标题 LLM 错误: {title} (save_title_only_samples=False)")
                else:
        ...
        if not passed and record_failed_sample:
            if is_title_only and not self._save_title_only_samples:
                if self._debug:
                    logger.info(f"[AI识别增强 自用定制版] 跳过保存仅标题样本: {title} (save_title_only_samples=False)")
            else:
    ...
    def on_chain_name_recognize(self, event) -> None:
        ...
        if not result.get("success"):
            if self._debug:
                logger.info(f"[AI识别增强 自用定制版] 跳过注入: {title or path} - {result.get('message')}")
            return
        guess = result.get("guess") or {}
        if isinstance(event_data, dict):
            if event_data.get("source_plugin"):
                if self._debug:
                    logger.info(f"[AI识别增强 自用定制版] 已有插件处理识别结果，跳过覆盖: {event_data.get('source_plugin')}")
                return
            ...
            event_data["source_plugin"] = "AIRecognizePro"
            event_data["confidence"] = guess.get("confidence", 0)
            event_data["reason"] = guess.get("reason", "")

        # 识别成功后异步沉淀识别词规则，不阻塞当前识别事件
        if self._auto_write_identifier:
            try:
                threading.Thread(
                    target=self._background_write_identifier,
                    args=(title, path, result),
                    daemon=True,
                ).start()
            except Exception as exc:
                logger.warning(f"[AI识别增强 自用定制版] 启动自动沉淀识别词任务失败: {exc}")

    def _background_write_identifier(self, title: str, path: str, result: Dict[str, Any]) -> None:
        """后台执行识别词沉淀，任何异常都不外抛。"""
        try:
            outcome = self._auto_write_identifier_rule(title=title, path=path, result=result)
            if outcome.get("success"):
                ...
                self.post_message(
                    ...
                    text=(
                        f"标题：{title}\n"
                        f"规则：{outcome.get('rule')}\n"
                        f"目标：{target.get('name')}（{target.get('year')}）\n"
                        f"当前识别词总数：{outcome.get('total_count')}"
                    ),
                )
            elif self._debug:
                logger.info(
                    f"[AI识别增强 自用定制版] 自动沉淀识别词未写入: {outcome.get('stage')} - {outcome.get('message')}"
                )
        except Exception as exc:
            logger.warning(f"[AI识别增强 自用定制版] 自动沉淀识别词后台任务异常: {exc}")
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        return form, {
            ...
            "auto_write_identifier": False,
            "auto_write_min_confidence": 0.8,
        }
