...
`AI识别增强 自用定制版` 是 `zhuzhug` 基于 `liuyuexi1987` 开源的「AI识别增强」做的 fork，用于自用定制，脱离官方升级覆盖。

**致谢**：感谢原作者 `liuyuexi1987` 的开源实现，本版本在其基础上补充了「识别成功自动沉淀识别词」能力。上游仓库：<https://github.com/liuyuexi1987/MoviePilot-Plugins>

本 fork 在原版之上增加了：

- 识别成功后自动生成窄作用域识别词，并去重写入 `CustomIdentifiers`
- 写入前做目标完整性校验（标题、年份、TMDB ID，剧集还要季号和集号）
- 写入后回放验证，不命中自动回滚，避免留下失效规则
- 置信度阈值过滤 + 并发写锁，防止识别事件并发时互相覆盖

`AI识别增强 自用定制版` 用来补强 MoviePilot 原生整理链里的识别阶段。
...
- 最新版 MoviePilot 下可以正常加载（独立插件 ID，不与官方版冲突）
- 正常中文标题识别可用
- 英文别名、韩文原名、中文别名可识别回标准媒体信息
- 低置信度标题会落失败样本
- `replay_failed_sample` 复查链可用
- 识别成功后可自动沉淀识别词，写入后回放不命中会自动回滚
